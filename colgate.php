...
$dbname = "colgate_survey"; // The database name
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($product) {
        ...
        $stmt = $conn->prepare("INSERT INTO SurveyResponses (Product) VALUES (?)");
        if ($stmt) {
            $stmt->bind_param("s", $product);

            // Execute the statement
            if ($stmt->execute()) {
                $message = "Thank you for participating in our survey! Your favorite product type is: <strong>$product</strong>";
            } else {
        ...
        $message = "No product selected. Please go back and select your favorite product type.";
    }
} else {
